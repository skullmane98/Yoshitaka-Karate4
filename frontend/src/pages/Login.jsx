    ...
    <PublicLayout>
      <section className="max-w-md mx-auto px-6 py-20" data-testid="login-page">
        <div className="text-center mb-10">
          ...
          <h1 className="font-serif text-5xl tracking-tight">{t("login.title")}</h1>
          <p className="text-[var(--dojo-ink-soft)] mt-2 text-sm">{t("login.subtitle")}</p>
        </div>
        <form onSubmit={submit} className="space-y-5 border border-[var(--dojo-border)] bg-[var(--dojo-paper)] p-8">
          <div>
            <label className="text-[10px] uppercase tracking-[0.24em] text-[var(--dojo-ink-soft)] block mb-2">{t("login.identifier_label")}</label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="text-[10px] uppercase tracking-[0.24em] text-[var(--dojo-ink-soft)] block mb-2">{t("login.password_label")}</label>
            <input
              type="password"
              required
          ...
          <button type="submit" className="btn-primary w-full" disabled={loading} data-testid="login-submit-btn">
            {loading ? (waking ? "…" : "…") : t("login.submit")}
          </button>
          <div className="text-center text-sm text-[var(--dojo-ink-soft)] pt-2 flex items-center justify-center gap-4">
            <Link to="/forgot-password" className="ink-underline" data-testid="login-forgot-link">{t("login.forgot")}</Link>
