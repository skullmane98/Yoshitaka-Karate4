...
- **[2026-03-04] Feature batch — auto-username, ID-Card auto-issued-date, deactivation exemption, auto-deactivation sweep, permission catalog admin, attendance search & pagination**
  ...
  - **Deactivation exemption**: new `User.deactivation_exempt` column (default false). UserDrawer Profile tab has `[data-testid=user-deactivation-exempt-toggle]`. `UserUpdateRequest` now allows the field (fixed regression flagged by testing agent iteration_7 — pytest re-passed in retest).
  - **Auto-deactivation sweep**: new endpoints `GET/PUT/POST /api/settings/auto-deactivate` with fields `{days, metric: scan|login|either}`. `metric` guardrails: days=0 disables; super_admins are never touched; exempt users skipped; sweep uses `max(last_scan_at, last_login_at, created_at)` for freshness. Frontend card in Permissions tab lets super-admins configure + run the sweep manually.
  - **Permission catalog admin**: new `PermissionCatalogEntry` table + endpoints `GET/POST/PATCH/DELETE /api/permission-catalog`. Super-admins can hide built-in permissions from the UI (backend still enforces them) and add/delete custom bookkeeping tags. Custom tags cannot enforce anything — they're labels-only.
  - **Attendance search & pagination**: new client-side `AttendanceLog` sub-component with search input (name/member#/belt/role), date-from / date-to range filters, and 25/page pagination. Backend adds `User.last_scan_at` tracking so the auto-deactivation sweep can key off attendance.
  - Verified via testing_agent iteration_7 → one HIGH bug found (PATCH dropping `deactivation_exempt`) → fixed → iteration_8 retest confirmed pass. Self-verified EN + ES tab cycle produces 0 runtime errors.
...
- **[2026-03-04] Admins can now see + edit other admins**
  ...
  - Verified via curl: admin GET returns admin-role users, PATCH on peer admin succeeds, PATCH on super_admin → 403 "Admins cannot edit super-admins".
...
- **[2026-03-04] Training overlay for Add User**
  ...
  - Video-demo option was scoped out (no AI tool generates a 60-min training video, and Sora 2 is capped at short clips).
